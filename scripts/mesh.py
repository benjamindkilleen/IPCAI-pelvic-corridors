...
@click.option("--neg/--no-neg", default=True)
def mesh(
    path: str,
    ...
    vol = Volume.from_nifti(path, segmentation=True)
    mesh = vol.isosurface(value=threshold, convert_to_LPS=True)
    mesh.save(output)
    ...
    if output_path.is_dir():
        output = output / f"{input_path.stem}.stl"
    ...
    vol.SetOrigin(
        sign * np.sign(R[0, 0]) * t[0],
        sign * np.sign(R[1, 1]) * t[1],
        sign * np.sign(R[2, 2]) * t[2],
    )
    vol.SetSpacing(
        sign * abs(R[0, 0]),
        sign * abs(R[1, 1]),

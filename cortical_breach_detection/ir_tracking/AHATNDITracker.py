...
class DepthFilterKalman:
    ...
    def startup(self, value):
        ...
        self.Filter.x = np.array([[value], [0.0]])  # initial state (location and velocity)

        self.Filter.F = np.array([[1.0, 1.0], [0.0, 1.0]])  # state transition matrix

        self.Filter.H = np.array([[1.0, 0.0]])  # Measurement function
        self.Filter.P *= 1000.0  # covariance matrix
        self.Filter.R = 5  # state uncertainty
        self.Filter.Q = Q_discrete_white_noise(2, 0.1, 0.3)  # process uncertainty
...
class AHAT_NDIToolScene:
    ...
    def SearchTools(self, ToolSceneFrame: AHAT_NDIToolSceneFrame):
        ...
        for i in range(len(self.Tools)):
            Possible_Solutions, Err_Solutions = self.SearchTool(
                ToolSceneFrame, self.Tools[i], self.ToleranceSide, self.ToleranceAvg
            )
    ...
    def FilterResult(self, ToolSceneFrame: AHAT_NDIToolSceneFrame):
        ...
        for toolnum in range(len(self.Tools)):
            if not ToolSceneFrame.SearchingStatus[toolnum]:
                ...
                for pointnum in range(len(xyd_this_tool)):
                    ...
                    l = math.sqrt(
                        xyd_this_tool_filterd[pointnum][0] ** 2
                        + xyd_this_tool_filterd[pointnum][1] ** 2
                        + 1
                    )
                    xyz_this_tool.append(
                        np.array(
                            [
                                xyd_this_tool_filterd[pointnum][0] / l * (d_f + 5.82),
                                xyd_this_tool_filterd[pointnum][1] / l * (d_f + 5.82),
                                (d_f + 5.82) / l,
                            ]
                        )
                    )
                ...
                tranfiltered, err = UVD_RigidTransform(
                    self.Tools[toolnum].Points, np.array(xyz_this_tool)
                )
                # print(np.array(xyd_this_tool)-np.array(xyd_this_tool_filterd))
    ...
    def UnionSegmentation(self, ToolSceneFrame: AHAT_NDIToolSceneFrame):
        ...
        for tool_num in range(len(self.Tools)):
            ...
            for _sltemp in PossibleSolutionTool:
                temp_id_PossibleSolution.append(
                    np.prod([self.Prime_list_500[0][_itertemp] for _itertemp in _sltemp])
                )
            ...
            for _id_notreprtitive in temp_id_notepetitive:
                ...
                cache.append(
                    (
                        tool_num,
                        True,
                        temp_Transformatrix[_min_tot_pos],
                        temp_err[_min_tot_pos],
                        PossibleSolutionTool[_min_tot_pos],
                    )
                )
        ...
        while sum(solu_num) > 0:
            ...
            for cache_i in range((len(cache) - 1), -1, -1):
                if (
                    cache[cache_i][0] == this_chache[0]
                    or len(np.intersect1d(this_chache[4], cache[cache_i][4])) != 0
                ):
    ...
    def SearchTool(
        self,
        ToolSceneFrame: AHAT_NDIToolSceneFrame,
        Tool: NDI_Tool,
        ToleranceSide,
        ToleranceAvg,
    ):
        ...
        # search for sides
        Eligible_side = list(
            np.where([abs(i - Tool_Map[0, 1]) < ToleranceSide for i in Scene_Sides_length])[0]
        )
        if len(Eligible_side) == 0:
            return Possible_solutions, Err_solutions
        for i in Eligible_side:
            Current_searching_status.append(
                tuple(
                    [
                        [Scene_Sides[i].id_in, Scene_Sides[i].id_out],
                        [Scene_Sides_length[i] - Tool_Map[0, 1]],
                    ]
                )
            )
            Current_searching_status.append(
                tuple(
                    [
                        [Scene_Sides[i].id_out, Scene_Sides[i].id_in],
                        [Scene_Sides_length[i] - Tool_Map[0, 1]],
                    ]
                )
            )
                ...
                if not new_node_id in searched_id:
                    ...
                    for old_node_id in range(num_has_searched):
                        if searched_id[old_node_id] < new_node_id:
                            temp_err.append(
                                abs(
                                    Scene_Map[searched_id[old_node_id], new_node_id]
                                    - Tool_Map[old_node_id, num_has_searched]
                                )
                            )
                        else:
                            temp_err.append(
                                abs(
                                    Scene_Map[new_node_id, searched_id[old_node_id]]
                                    - Tool_Map[old_node_id, num_has_searched]
                                )
                            )
                    ...
                    Current_searching_status.append(
                        tuple(
                            [
                                list(np.concatenate((searched_id, [new_node_id]))),
                                list(np.concatenate((searched_err, temp_err))),
                            ]
                        )
                    )
            # break

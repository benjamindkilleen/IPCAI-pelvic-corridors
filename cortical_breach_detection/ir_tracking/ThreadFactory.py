...
class AHATIRToolTracking(threading.Thread):
    ...
    def __init__(self, _AHATCurrentFrame, Tools, ToolNames, lut, port, radius):
        super(AHATIRToolTracking, self).__init__()
        self.AHATCurrentFrame = _AHATCurrentFrame
        self.sphere_radius = radius  # unit: mm
        ...
        self.ToolNamesMap = dict(zip(ToolNames, range(len(ToolNames))))
        self._start = False
        ...
        self.flipz[1, 2] = -1
        self.flipz[2, 3] = -1
        self.flipz[3, 2] = -1  # doesn't do much, since last row is always 0, 0, 0, 1

        # buffers for publishing tool pose data back to the main thread
        self.tool_pose_locks = [threading.Lock() for _ in range(len(Tools))]
        self.tool_poses = np.empty((len(Tools), 4, 4))
        self.tool_poses_updated = np.zeros((len(Tools),), dtype=bool)
    ...
    def run(self):
        ...
        while True:
            ...
            _im = (
                (_im - self._downlim)
                * (256 * 256 / self._upperlim)
                / (self._upperlim - self._downlim)
                * 255
            )
            ...
            retval, labels, stats, centroids, label_corr = DetectBalls(
                _mask,
                self._minSize,
                self._maxSize,
                self._distanceTolerance,
                self._preretval,
                [],
                [],
                self._precentroids,
            ...
            for j in range(centroids.shape[0]):
                ...
                _d = _DepthFrame[int(_v), int(_u)] + self.sphere_radius
            ...
            _tempScene = AHAT_NDIToolSceneFrame(
                np.array(_xyz), np.array(_xyd), _timestamp=Curr_fr.timestamp
            )
            ...
            for i in range(len(_tempScene.TransformatrixFiltered)):
                if not _tempScene.SearchingStatus[i]:
                    continue

                # get tool in world, unity coordinates
                tool_pose_rhs = _tempScene.TransformatrixFiltered[i]
                ...
                toolInWorld_mm = toolInWorld_rhs.copy()
                toolInWorld_mm[:3, 3] = toolInWorld_mm[:3, 3] * 1000.0
                with self.tool_pose_locks[i]:
                    self.tool_poses[i] = toolInWorld_mm
                    self.tool_poses_updated[i] = True

                with self.pub_lock:
                    p_msg = toolInWorld_lhs[:3, 3].astype("<f").tobytes()
                    R_msg = R.from_matrix(toolInWorld_lhs[:3, :3]).as_quat().astype("<f").tobytes()
                    idx_msg = np.array(i).astype("<i").tobytes()
                    self.socket_pub.send_multipart([b"ir_tool", p_msg, R_msg, idx_msg])

    def get_poses(self) -> Dict[str, np.ndarray]:
        """Get the list of tool pose transforms (world from tool).

        Returns:
            Dict[str, np.ndarray]: A dictionary of tool name to pose transform.
        ...
        for i in range(len(self.ToolNames)):
            with self.tool_pose_locks[i]:
                if self.tool_poses_updated[i]:
                    tool_poses[self.ToolNames[i]] = self.tool_poses[i]
                    self.tool_poses_updated[i] = False
        ...
        try:
            ...
            while True:
                ...
                if (not self.RawAHATQueue.empty()) and (not self.AHATQueue.full()):
                    ...
                    imgab = np.frombuffer(_RawAHATFrame.RawReflectivity, dtype="uint16").reshape(
                        512, 512
                    )
                    imgdepth = np.frombuffer(_RawAHATFrame.RawDepth, dtype="uint16").reshape(
                        512, 512
                    )
                    ...
                    imgab_processed = np.uint8(
                        (imgab_processed - Ab_low) / (Ab_high - Ab_low) * 255
                    )

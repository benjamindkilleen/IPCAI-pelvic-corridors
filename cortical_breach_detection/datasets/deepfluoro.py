...
class RandomDataAugDataSet(torch.utils.data.Dataset):
    def __init__(
        ...
        lands=None,
        proj_pad_dim=0,
        do_norm_01_scale: bool = True,
        hist_reference: Optional[str] = None,
    ):
        self.projs = projs
        self.segs = segs
        self.lands = lands
        ...
        self.do_norm_01_scale = do_norm_01_scale

        self.include_heat_map = True

        self.print_aug_info = False

        if hist_reference is not None:
            self.hist_reference = np.array(Image.open(hist_reference)).astype(np.float32) / 255.0
        else:
            self.hist_reference = None
    ...
    def __getitem__(self, i):
        ...
        if (self.prob_of_aug > 0) and (random.random() < self.prob_of_aug):
            ...
            if self.do_noise:
                ...
                if self.print_aug_info:
                    log.info("noise sigma: {:.3f}".format(cur_noise_sigma))
            ...
            if self.do_gamma:
                ...
                if self.print_aug_info:
                    log.info("gamma = {:.2f}".format(gamma))
            ...
            if self.do_affine:
                ...
                if self.print_aug_info:
                    log.info("Rot: {:.2f}".format(rot_ang))
                    log.info("Trans X: {:.2f} , Trans Y: {:.2f}".format(trans_x, trans_y))
                    log.info("Shear X: {:.2f} , Shear Y: {:.2f}".format(shear_x, shear_y))
                    log.info("Scale: {:.2f}".format(scale_factor))
                ...
                if cur_lands is not None:
                    ...
                    A = np.matrix(
                        [[A_inv[0], A_inv[1], A_inv[2]], [A_inv[3], A_inv[4], A_inv[5]], [0, 0, 1]]
                    ).I
            ...
            if self.do_erase and (random.random() < self.erase_prob):
                ...
                if self.print_aug_info:
                    log.info("  Random Corrupt: num. boxes: {}".format(num_boxes))
        ...
        if self.include_heat_map:
            ...
            (Y, X) = torch.meshgrid(
                torch.arange(0, s.shape[-2]), torch.arange(0, s.shape[-1]), indexing="ij"
            )
            Y = Y.float()
            X = X.float()
        ...
        image = p.to(torch.float32)
        target = dict(
            heatmaps=h.to(torch.float32),
            segs=s.to(torch.float32),
        )
        info = dict(
            left_side=torch.tensor(1, dtype=torch.long),
            cur_lands=cur_lands.to(torch.float32),
        )

        # log.debug(f"image: {image.mean()}, {image.std()}, ({image.min()}, {image.max()})")
        # TODO: histogram matching on image.
        if self.hist_reference is not None:
            img = utils.get_numpy(image[0])
            img = match_histograms(img, self.hist_reference)
            image = torch.from_numpy(img[np.newaxis, :, :])

        # log.debug(f"matched: {image.mean()}, {image.std()}, ({image.min()}, {image.max()})")
...
def get_dataset(
    ...
    **kwargs,
    ...
    Args:
        h5_file_path: path to the h5 file containing the data.
        pat_inds: list of patient indices to include in the dataset, in `[1,2,3,4,5,6]`.
        num_classes: number of classes in the segmentation.
        pad_img_dim: number of pixels to pad the image with.
        no_seg: if True, don't include the segmentation. (not used)
        minmax: if not None, use this minmax to normalize the data. If True, use the minmax of the data. Default: None.
        data_aug: if True, do data augmentation. Default: False.
        train_valid_split: if not None, split the data into train and valid. If None, return the whole dataset. Default: None.
        train_valid_idx: if not None, use this to split the data into train and valid. If None, use the indices in `pat_inds`. Default: None.
        dup_data_w_left_right_flip: if True, duplicate the data with left and right flipped images. Default: False.
    ...
    if minmax is not None:
        if (type(minmax) is bool) and minmax:
            ...
            log.info("need to find min/max for preprocessing...")
    ...
    if need_to_scale_data:
        ...
        log.info("scaling data using min/max: {} , {}".format(minmax_min, minmax_max))
    ...
    if (train_valid_split is not None) and (train_valid_split > 0):
        log.info("split dataset into train/validation")
        ...
        if (
            ...
            log.info("  randomly splitting all complete tensors into training/validation...")
            random.shuffle(all_inds)

            train_inds = all_inds[:num_train]
            valid_inds = all_inds[num_train:]
        else:
            log.info("  use previously specified split")
        ...
        ds = RandomDataAugDataSet(
            all_projs, all_segs, all_lands, proj_pad_dim=pad_img_dim, **kwargs
        )
...
class DeepFluoroDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        log.info("TODO: automatically download IPCAI fluoro data if not present")

    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            raise ValueError("this module does not support training")

        if stage == "test" or stage is None:
            self.test_set = get_dataset(**self.dataset_kwargs)
